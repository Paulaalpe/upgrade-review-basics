...
/***Iteración #2: Mix Fors**    Dado el siguiente javascript usa forof y forin para hacer la media del volumen de todos los sonidos favoritos que tienen los usuarios */
...
for(let user of users){
    ...
    for(let clave in sonidosFavoritos){
        let formatoYVolumen = sonidosFavoritos[clave];
        volumenes.push(formatoYVolumen.volume);      
    }
};

let totalVolumenes = volumenes.length;
let sumaVolumenes = 0;
for(let i=0;i<volumenes.length;i++){
    sumaVolumenes += volumenes[i];  
}
let mediaVolumenes = sumaVolumenes / totalVolumenes;
console.log(mediaVolumenes);
